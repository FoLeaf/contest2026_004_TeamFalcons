...
10. [ ] Track required CI and review feedback on `open-vela/nuttx#323` and
    `open-vela/nuttx-apps#112`; fix verified findings without mixing unrelated
    QSPI or FT5X06 changes into either PR.
11. [ ] Wait for both PRs to merge into `dev-ai-contest-2026`, then run
    `repo sync` so the parent workspace consumes the official commits.
12. [ ] Disable the matching temporary patch injection in the team build,
    rebuild from synced sources, run the hardware smoke matrix, and only then
    perform the final Trellis archive.
