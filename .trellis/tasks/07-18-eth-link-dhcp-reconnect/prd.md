...
- [x] 静态回归能断言 link polling、carrier on/off、DHCP 重连和 B01
  QSPI 共脚保护合同。
- [ ] `open-vela/nuttx#323` 与 `open-vela/nuttx-apps#112` 的必需 CI、评审
  和合入全部完成，目标分支均为 `dev-ai-contest-2026`。
- [ ] 两个 PR 合入后执行 `repo sync`，停用团队仓内对应的临时补丁注入，
  再次完成构建和硬件冒烟验证，确认交付依赖官方源码而非本地 patch。
