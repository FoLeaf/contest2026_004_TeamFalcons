...
- [x] 保存阶段 2 与基线的对照，明确仍待交互阶段解决的项目。（见 `research/final-results.md`；L1+ 仍归子任务 3 T5）

## 阶段 3：交互与最终证据

- [x] 在阶段 2 通过后实施 `09-13-hmi-touch-acceptance`（自动部分 T1–T4；T5 真机交互未完）。
- [x] 验证输入周期、按下反馈、手势取消、动态列表和返回状态。（代码 + headless；实体触摸未测）
- [ ] 完成统一验收契约要求的全部场景及样本数。（缺 L1+ 实体触摸/录像）
- [ ] 对 Agent 负载失败核对既有系统挂死任务，不通过停用产品能力制造通过结果。
- [x] 检查所有新增界面状态使用真实数据，未解锁延期功能或新增权限。
